=begin
Write a method that returns an Array that contains every other element
of an Array that is passed in as an argument. The values in the returned
list should be those values that are in the 1st, 3rd, 5th, and so on elements
of the argument Array.

Examples
oddities([2, 3, 4, 5, 6]) == [2, 4, 6]
oddities([1, 2, 3, 4, 5, 6]) == [1, 3, 5]
oddities(['abc', 'def']) == ['abc']
oddities([123]) == [123]
oddities([]) == []

Understand the problem:
- Input: array
- output: array
- Write a method, `oddities`
- Method takes one argument, an array `arr`
- Return a new array, `result`
- `result` should contain every other element from original array `arr`
  - `result` should contain element 0, 2, 4, 6, etc
- When given empty array should return empty arraoy

Algorithm
- Loop through each element in the array

=end

def oddities(arr)
  result = []
  arr.each_with_index { |item, index| result << item if index.even? }
  result
end

# Test cases
p oddities([2, 3, 4, 5, 6]) == [2, 4, 6] # => true
p oddities([1, 2, 3, 4, 5, 6]) == [1, 3, 5] # => true
p oddities(['abc', 'def']) == ['abc'] # => true
p oddities([123]) == [123] # => true
p oddities([]) == [] # => true