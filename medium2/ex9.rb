=begin
Bubble Sort is one of the simplest sorting algorithms available. It isn't an
efficient algorithm, but it's a great exercise for student developers. In this
exercise, you will write a method that does a bubble sort of an Array.

# A bubble sort works by making multiple passes (iterations) through the Array.
On each pass, each pair of consecutive elements is compared. If the first of
the two elements is greater than the second, then the two elements are swapped.
This process is repeated until a complete pass is made without performing any
swaps; at that point, the Array is completely sorted.
    
# We can stop iterating the first time we make a pass through the array without
making any swaps; at that point, the entire Array is sorted.

# Write a method that takes an Array as an argument, and sorts that Array using
the bubble sort algorithm as just described. Note that your sort will be
"in-place"; that is, you will mutate the Array passed as an argument. You may
assume that the Array contains at least 2 elements.

PROBLEM:
- in: array (at least 2 elements, integers or strings)
- output: same array, sorted low to high
- bubble sort
  - Consecutive elements are compared (<=>)
  - If the left element is greater than the right, swap them elements. Otherwise, leave them
  - If a pass (an iteration through all the elements comparing them) leads to no swaps, array is sorted
  
Approach:
- swaps = false
- repeat until an pass that has no swaps
  - any swap, turn swaps to true
  
[6, 2, 7, 1, 4] 
6 2 - compare
2 6 - swap
record that I swapped

greater <=> less => 1
less <=> greater => -1
equal <=> equal => 0

index = 0
6 (0+1) 2 <=> # => 1


Algorithm:
- initialize a swaps variable to false
- loop (do while)
  - iterate through all the elements with the index
    - go to the next iteration if index == arr.length
    - compare element at current index to element at index + 1
      - if the comparison returns 1, then swap
        - arr[index], arr[index + 1] = arr[index + 1], arr[index]
        - reassign swaps to true
      - otherwise, do nothing
  - break if swaps == false
- return the array

=end

def bubble_sort!(arr)
  loop do
    swaps = false
    arr.each_with_index do |elem, index|
      next if index == arr.length
      if (elem <=> arr[index + 1]) == 1
        arr[index], arr[index + 1] = arr[index + 1], arr[index]
        swaps = true
      end
    end
    break if swaps == false
  end
end

# array = [1, 2, 3]
# bubble_sort!(array)
# p array == [1, 2, 3]

array = [5, 3]
bubble_sort!(array)
p array == [3, 5]

array = [6, 2, 7, 1, 4]
bubble_sort!(array)
p array == [1, 2, 4, 6, 7]

array = %w(Sue Pete Alice Tyler Rachel Kim Bonnie)
bubble_sort!(array)
p array == %w(Alice Bonnie Kim Pete Rachel Sue Tyler)