...
Problem:
- input: three integers (three angles of a triangle)
...
def triangle(angle1, angle2, angle3)
  ...
  case
  when triangle.sum != 180 || triangle.any? { |angle| angle <= 0 } then :invalid
  when triangle.all? { |angle| angle < 90} then :acute
  when triangle.any? { |angle| angle > 90} then :obtuse
  else :right
  end
end
